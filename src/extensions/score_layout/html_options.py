...
def return_html_theme_options(app: Sphinx) -> dict[str, object]:
    ...
    if app.config.html_context.get("github_user") != "default" and app.config.html_context.get("github_repo") != "default":
        ...
        theme_options["navbar_end"] = ["theme-switcher", "navbar-icon-links", "version-switcher"]
    else:
...
def return_html_context(app: Sphinx) -> dict[str, str]:
    ...
    if not hasattr(app.config, 'html_context') or (not app.config.html_context.get("github_user") and not app.config.html_context.get("github_repo")):
        return {
